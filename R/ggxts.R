...
#' @export
#'
ggxts = function(x, digits = 2){
  assertClass(x,"xts")

  df = data.frame(Index = index(x), data.frame(x))
  ...
  ggplot(df2, aes(x = Index, y = value, group = variable, color = variable)) + geom_line() + ylab(NULL) + xlab(NULL) +
